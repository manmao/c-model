	...
	,m_stop(false)
{
	...
	m_sub_process=new process[process_number]; //create n 
...
void processpool<T>::run_child()
{
	...
	while(!m_stop)
	{
		...
		for(int i=0;i<number;i++)
		{
			...
			else if(events[i].events & EPOLLIN)  //connfd 
			{
				user[sockfd].process();
			}
			else
	...
	while(!m_stop)
	{
		...
		for(int i=0;i<number;i++)
		{
			...
			if(sockfd == m_listenfd)
			{
				...
				do
				{
					if(m_sub_process[i].m_pid != -1)  // free process 
						break;
